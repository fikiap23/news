<x-livewire-tables::bs5.table.cell>
    <div class="d-flex align-items-center">
        <div class="me-5">
            <img src="{{ $row->profile_image }}" alt="" width="50" height="50"
                class="rounded-circle object-cover">
        </div>
        <div class="d-flex justify-content-start flex-column">
            <span class="text-muted fw-bold text-muted d-block fs-7">{!! $row->first_name !!}
                {!! $row->last_name !!}</span>
...
<x-livewire-tables::bs5.table.cell>
    ...
    <label
        ...
        <input type="checkbox" name="status" class="form-check-input user-active cursor-pointer"
            wire:change="emailVerified({{ $row->id }})" value="{{ $row->email_verified_at }}"
            {{ $row->email_verified_at != null ? 'checked' : '' }}
            {{ $row->email_verified_at != null ? 'disabled' : '' }}>
...
<x-livewire-tables::bs5.table.cell :customAttributes="['class' => 'custom-width-album']">
    <div class="d-flex justify-content-start">
        @if ($row->email_verified_at == null)
            <a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover"
        ...
        @endif
        <a href="{{ route('staff.edit', $row['id']) }}" data-bs-toggle="tooltip" data-bs-placement="top"
            data-bs-trigger="hover" data-bs-original-title="{{ __('messages.common.edit') }}"
