                        ...
                        @foreach (getCategory()->take(6) as $category)
                            <li>
                                <a href="{{ route('categoryPage', $category->slug) }}"
                                    class="text-decoration-none mb-3 d-block text-gray fs-14 text:hover">{!! $category->name !!}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
                    ...
                    @foreach (getRecentPost() as $recentPost)
                        <div class="card me-sm-0 me-4  mb-4 bg-light {{ $loop->index ? 'mb-sm-0' : '' }}">
                            ...
                            <div class="card-body">
                                ...
                                <span
                                    class="card-text fs-12 text-gray">{{ $recentPost->created_at->format('M d Y') }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
        ...
        <div class="last-line pt-60 pb-4">
            <div class="row justify-content-between align-items-center">
                <div class="col-xxl-3 col-lg-4 col-sm-6 text-lg-start text-sm-end text-center order-2 order-lg-0">
                    <p href="#" class="fs-12 text-gray mb-0">{{ __('messages.common.all_rights') }} ©
                        {{ Illuminate\Support\Carbon::now()->format('Y') }} {{ $settings['application_name'] }}</p>
                </div>
                <div
                    ...
                    <div
                        ...
                        <a href="{{ $settings['facebook_url'] }}" target="_blank"> <i
                                class="fa-brands fa-facebook-f text-gray fs-18 me-xl-5 me-4"></i> </a>
                        <a href="{{ $settings['twitter_url'] }}" target="_blank"> <i
                                class="fa-brands fa-twitter text-gray fs-18 me-xl-5 me-4"></i> </a>
                        <a href="{{ $settings['linkedin_url'] }}" target="_blank"> <i
                                class="fa-brands fa-linkedin-in  text-gray fs-18 me-xl-5 me-4"></i></a>
                        <a href="{{ $settings['pinterest_url'] }}" target="_blank"> <i
                                class="fa-brands fa-pinterest text-gray fs-18 me-xl-5 me-4"></i></a>
                        <a href="{{ $settings['instagram_url'] }}" target="_blank"> <i
                                class="fa-brands fa-instagram  text-gray fs-18  me-xl-5 me-4"></i></a>
                        <a href="{{ $settings['vk_url'] }}" target="_blank"> <i
                                class="fa-brands fa-vk text-gray fs-18  me-xl-5 me-4"></i></a>
                        <a href="{{ $settings['telegram_url'] }}" target="_blank"> <i
                                class="fa-brands fa-telegram  text-gray fs-18  me-xl-5 me-4"></i></a>
                ...
                <div class="col-xxl-3 col-lg-4 col-sm-12 text-lg-end mb-lg-0 mb-sm-4 text-center order-0 order-lg-2">
                    <div class="desc  justify-content-center ">
                        <a href="{{ route('page.Terms') }}"
                            class="fs-12 me-4 {{ Request::is('terms-conditions*') ? 'text-success' : 'text-gray' }}">{{ __('messages.setting.terms-conditions') }}</a>
                        <a href="{{ route('page.support') }}"
                            class="fs-12 me-4 {{ Request::is('support*') ? 'text-success' : 'text-gray' }}">{{ __('messages.setting.support') }}</a>
                        <a href="{{ route('page.privacy') }}"
                            class="fs-12 {{ Request::is('privacy*') ? 'text-success' : 'text-gray' }}">{{ __('messages.setting.privacy') }}</a>
                    </div>
                </div>
            </div>
        </div>
